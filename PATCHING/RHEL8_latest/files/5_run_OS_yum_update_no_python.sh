...
#
# RHEL7
#yum -y --disablerepo=* --enablerepo=rhel-7-server-rpms --enablerepo=rhel-7-server-optional-rpms --enablerepo=rhel-7-server-extras-rpms install kernel-3.10.0-1160.21.1.el7 >> $update_log 2>&1

# RHEL8
yum -y --disablerepo=* --enablerepo=rhel-8-for-x86_64-baseos-rpms --enablerepo=rhel-8-for-x86_64-appstream-rpms install kernel-4.18.0-305.25.1.el8 >> $update_log 2>&1
if [ $? -ne 0 ]
then
...
yum -y --disablerepo=* --enablerepo=rhel-8-for-x86_64-baseos-rpms --enablerepo=rhel-8-for-x86_64-appstream-rpms --exclude=kernel update-to kernel-tools-4.18.0-305.25.1.el8.x86_64 kernel-tools-lib-4.18.0-305.25.1.el8.x86_64
if [ $? -ne 0 ]
then
...
echo "" >> $update_log 2>&1
# NOTE the exclude on the command line below because it is installed rather than updated in the above

#yum -y --disablerepo=* --enablerepo=rhel-7-server-rpms --enablerepo=rhel-7-server-optional-rpms --enablerepo=rhel-7-server-extras-rpms --exclude=kernel --exclude=kernel-tools --exclude=kernel-tools-libs update >> $update_log 2>&1

yum -y --disablerepo=* --enablerepo=rhel-8-for-x86_64-baseos-rpms --enablerepo=rhel-8-for-x86_64-appstream-rpms --exclude=kernel --exclude=kernel-tools --exclude=kernel-tools-libs --exclude=python --exclude=python-lib update >> $update_log 2>&1
